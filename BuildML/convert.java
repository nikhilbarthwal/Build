...
public class convert
{
    ...
    {
        System.out.print("Process File: " + f);
        ...
        for(String s : Files.readAllLines(Paths.get(f), Charset.defaultCharset()))
        {
            ...
            for(int i = 0; i<s.length(); i++)
            {
                int k = (int) s.charAt(i);
                     if (k==9) { p+=4; S.push(sp); S.push(sp); S.push(sp); S.push(sp); }
        ...
        while ( (n>0) && (data.get(n-1) == nl)) n--;

        if (n==0) { System.out.println("  :: skipping empty file"); return; }
        ...
        System.out.println("  ::    " + data.size() + " bytes written ");
    }
}
