...
Here pattern consist of unbounded identifier packaged in structs/tuples/adt but is immutable. Type is optional.
...
Define a function (Same as above) but this is a block with last expression evaluated and all other expression should evaluate nil. types are optional
...
*if {bool expr} then: // Can precede with def or let definitions, without else is only valid for void*
...
If then else definitions. These are expressions and would proceed with let /var/ def but not def block. Notice that end is only needed when last clause is block.
...
This is an expression. The identifier here is a tuple which the functions takes and evaluates as arguments.
