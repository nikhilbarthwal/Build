...
(*
RefinesSourceOutput = (ErrorList, LowerPairs, UpperPairs)
{
    AddError -> SourceParseOutput
    AddLowerPain -> SourceParseOutput
    AddUpperPain -> SourceParseOutput
}

type DuplicateSourceTree = (Map<String, ErrorList>, LowerPairs, UpperPairs)
{
    Add(Filename, SourceParseOutput(ErrorList, LowerPairs, UpperPairs)) -> DuplicateSourceTree
    
    GetLowerMap() -> (LowerMap, DuplicateErrors)
    GetUpperMap() -> (UpperMap, DuplicateErrors)

    static Intial -> DuplicateErrors
}

Program(f: File List) // All errors or Output
{
    DuplicateSourceTree = f |> List.fold (fun f s -> s.Add(f, ReadFile[f])) Intial
    
    lowermap, errors1 = DuplicateSourceTree.GetLowerMap()
    uppererror, errors2 = DuplicateSourceTree.ErrorLowerMap()

    int Errors = DuplicateSourceTree.Errors.size() + error1.size() + error2.size()

    if (Errors = 0)
    {
        errors = Verify(LowerMap, UpperMap)

        int n2 = errors.size
        if (n2==0)
            return Tranform((LowerMap, UpperMap)
        else
            print errors
            return Right[n2]
    }
...
Program = Tranform((LowerMap, UpperMap)
{
...
Program -> Execute functions here
