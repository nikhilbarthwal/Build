:-module(my_list,[list_init/3,array/3,list_verify/2,list_replace/4,list_replace_all/4,list_substitute/4,list_extract/4,	list_get/3,
         list_put/4, list_include/4, list_exclude/4, list_gen/3, list_gen/4, list_multiplex/3, list_mult/2, list_mult/3,list_member/2]).

:-use_module(library(lists)). 

:- meta_predicate list_include(:,+,+,?), list_exclude(:,+,+,?).
...
list_include_tmp(_,_,_,[],L,L).
list_include_tmp(U,G,M,[H|T],L1,L3) :- G =.. [GH|GT1], list_replace_all(H,M,GT1,GT2), GG =.. [GH|GT2], CG =.. [:,U,GG],
                                       ( call(CG) -> L2 = [H|L1] ; L2 = L1 ), list_include_tmp(U,G,M,T,L2,L3).
list_include(G,M,L,LL) :- \+ var(M), \+ var(L), is_list(L), list_include_tmp(G,M,L,[],LLL), reverse(LLL,LL), !.

list_exclude_tmp(_,_,_,[],L,L).
list_exclude_tmp(U,G,M,[H|T],L1,L3) :- G =.. [GH|GT1], list_replace_all(H,M,GT1,GT2), GG =.. [GH|GT2], CG =.. [:,U,GG],
                                       ( call(CG) -> L2 = L1 ; L2 = [H|L1] ), list_exclude_tmp(U,G,M,T,L2,L3).
list_exclude(G,M,L,LL) :- \+ var(M), \+ var(L), is_list(L), G =.. [:,U,GG], list_exclude_tmp(U,GG,M,L,[],LLL), reverse(LLL,LL), !.
